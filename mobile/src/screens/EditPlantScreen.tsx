...
import { UpdatePlantDto, PlantSpecies } from '../types/plant';
  ...
  const loadPlant = async () => {
    try {
      ...
      if (plant.species) {
        ...
        setSpeciesQuery(plant.species.commonName);
      }

      if (plant.acquisitionDate) {
        setSelectedDate(new Date(plant.acquisitionDate));
      }
    } catch (error: any) {
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              {formData.photos?.map((photo, index) => (
                <View key={index} style={styles.photoWrapper}>
                  <Image source={{ uri: uploadService.toFullUrl(photo) }} style={styles.photoThumbnail} />
                  <TouchableOpacity
